...
schtasks /Create /F /TN "ONT Restart Bot" /TR "\"%~dp0start_bot.bat\"" /SC ONLOGON /RU "%USERNAME%" /RL LIMITED
if errorlevel 1 (
    echo.
    echo   [!] Task ONLOGON butuh admin - gagal. Pakai Startup folder sebagai gantinya...
    call "%~dp0install_startup.bat"
)
...
echo (atau via Startup folder kalau Task Scheduler butuh admin).
